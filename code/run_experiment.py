...
def RunExperiment(pathData, pathLabel, f_select, model, evl, ensemble_n=30, n_splits=10,  f_scaling = "None"):
    """ 
    ...
    for train_index, test_index in kf.split(X):
        ...
        for i in range(ensemble_n):
            if model == "rf":
                ...
                clf[i] = SGDClassifier(loss='log', penalty='elasticnet',alpha=0.1, max_iter=10000)
                clf[i].fit(training[i][:,features],label[i])
